...
const scheduleDetailSections = () => {
  ...
  visibleDetailSections.value = 0;

  if (props.dataOnly || isLoading.value) return;
  ...
  const revealNext = () => {
    window.setTimeout(() => {
      ...
      visibleDetailSections.value += 1;
      if (visibleDetailSections.value < 5) {
        revealNext();
      }
    }, visibleDetailSections.value === 0 ? 100 : 50);
  };
    ...
    if (newLeagueInfo) {
      if (
        ...
        await inputLeague(
          ...
          newLeagueInfo.platform === "espn" ? "espn" : "sleeper"
        );
      } else {
...
watch(
  [isLoading, () => props.dataOnly, () => store.currentLeagueId],
...
const tableDataAllYears = computed(() => {
  ...
  > = {
    wins: (a, b) => {
      ...
      return ratioB !== ratioA ? ratioB - ratioA : b.points - a.points;
    },
    points: (a, b) =>
      b.points / (b.wins + b.losses) - a.points / (a.wins + a.losses),
    expectedWins: (a, b) =>
      b.wins - b.randomScheduleWins - (a.wins - a.randomScheduleWins),
    managerEfficiency: (a, b) =>
      b.managerEfficiency / (b.seasons.length + currentLeague.value) -
      a.managerEfficiency / (a.seasons.length + currentLeague.value),
  };

  return data.sort(sorters[tableOrder.value] ?? (() => 0));
});
...
<template>
  <div v-if="!props.dataOnly" class="my-4">
    <ScrollableTableCard
      v-if="!isLoading"
              ...
              >
                <Tooltip>
                  ...
                  <TooltipContent class="bg-muted-foreground w-52">
                    Actual wins minus expected wins against a uniformly random
                    opponent each week
                  </TooltipContent>
                </Tooltip>
              </th>
            ...
            <tr v-for="(user, index) in tableDataAllYears" class="border-b">
              ...
              <td
                ...
                :class="{
                  'text-primary font-semibold':
                    Math.round(
                      (user.points / (user.wins + user.losses)) * 100
                    ) /
                      100 ===
                    mostPoints,
                  'text-destructive font-semibold':
    ...
    <SeasonFinishHistory
      v-if="!isLoading && visibleDetailSections >= 1"
      :tableData="props.tableData"
    />
    <AllMatchups
      v-if="!isLoading && visibleDetailSections >= 2"
      :tableData="dataAllYears"
      class="mt-4"
    />
    <AllTimeRecords
      v-if="!isLoading && visibleDetailSections >= 3"
    ...
    <div v-if="!isLoading && visibleDetailSections >= 5">
