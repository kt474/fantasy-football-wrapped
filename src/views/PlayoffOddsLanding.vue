...
const forecastStages = [
  {
    title: "League state",
    detail:
      "Current record, points scored, completed weeks, playoff cutoff, and the imported remaining schedule when available.",
  },
  {
    title: "Team strength",
    detail:
      "Recent scoring form and projected starter output using the league’s starting roster slots and scoring format.",
  },
  {
    title: "5,000 seasons",
    detail:
      "The remaining schedule is replayed with weekly scoring variation, head-to-head matchups, and median scoring when enabled.",
  },
  {
    title: "Probability range",
    detail:
      "Playoff odds, top-seed odds, average seed, expected wins, and a likely win range for every manager.",
  },
];
...
const views = [
  {
    ...
    description:
      "Follow the expected win race week by week and compare wins already secured with the simulated final total.",
  ...
  },
  {
    ...
    description:
      "Replay or rearrange schedules to see how much a manager’s record and seed depend on opponent timing.",
  ...
  },
  {
    ...
    description:
      "See a range of plausible finishes instead of treating one projected record as a guaranteed outcome.",
...
const faqs = [
  {
    ...
    answer:
      "ffwrapped combines the current league state, remaining schedule, scoring history, roster projections, playoff cutoff, and league rules, then runs 5,000 simulated finishes for the active-season forecast.",
  },
  {
    question: "Do the odds use my league’s real remaining matchups?",
    answer:
      "Yes. The season forecast begins with the imported schedule when it is available. Scenario tools can also show how standings change under alternate schedule arrangements.",
  },
  {
    question: "Are playoff probabilities guaranteed?",
    answer:
      "No. They are estimates based on projections and scoring variation. Injuries, lineup changes, trades, and unpredictable weekly results can move the forecast quickly.",
  },
  {
    question: "Does the forecast support median scoring?",
    answer:
      "The simulation accounts for the league’s median-scoring setting when it is enabled and applies those weekly results alongside head-to-head matchups.",
  },
];
</script>
  ...
  <PublicPageShell>
      ...
      >
        <div>
          ...
          <p class="max-w-2xl mt-5 text-lg leading-8 text-muted-foreground">
            ...
            probability, projected wins, likely seed, and the range of finishes
            still open to every team.
          </p>
          <div class="flex flex-wrap gap-3 mt-8">
            <Button as-child size="lg"
              ><RouterLink to="/">Calculate playoff odds</RouterLink></Button
            >
            <Button as-child size="lg" variant="outline"
              ><RouterLink to="/sleeper-league-analyzer"
                >Explore league analysis</RouterLink
              ></Button
            >
          </div>
          <div
          ...
          >
            ...
            >
            <span class="inline-flex items-center gap-1.5"
              ><Check :size="14" /> Real remaining schedule</span
            >
            <span class="inline-flex items-center gap-1.5"
              ><Check :size="14" /> League specific scoring</span
            >
          </div>
        </div>
    ...
    <section class="max-w-6xl px-5 py-16 mx-auto sm:py-20">
      <div class="max-w-3xl">
        ...
        <h2 class="mt-2 text-3xl font-semibold tracking-tight">
          From current standings to a range of possible finishes
        </h2>
        <p class="mt-4 leading-7 text-muted-foreground">
          A probability becomes more useful when the inputs and outputs are
          visible. The forecast keeps the real league structure intact
          throughout the simulation.
        </p>
      </div>
      <ol
        class="grid mt-10 overflow-hidden border divide-y rounded-card lg:grid-cols-4 lg:divide-x lg:divide-y-0"
      >
        <li
          v-for="(stage, index) in forecastStages"
          :key="stage.title"
          class="p-6"
        >
          <div class="flex items-center justify-between">
            ...
            >
            <ArrowRight
              v-if="index < forecastStages.length - 1"
              :size="15"
      ...
      >
        <div class="lg:sticky lg:top-24 lg:self-start">
          <p class="text-sm font-medium text-primary">
            Three ways to read the race
          </p>
          <h2 class="mt-2 text-3xl font-semibold tracking-tight">
            Probability, trajectory, and schedule luck
          </h2>
          <p class="mt-4 leading-7 text-muted-foreground">
            Each view answers a different question, so the same percentage does
            not have to carry the entire playoff story.
          </p>
        </div>
        <div class="border-t">
          <article
            v-for="view in views"
            :key="view.title"
            class="grid gap-4 py-7 border-b sm:grid-cols-[2rem_minmax(0,1fr)]"
          >
    ...
    <section class="max-w-6xl px-5 py-16 mx-auto sm:py-20">
      <div class="grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center">
        <div>
          <p class="text-sm font-medium text-primary">
            Read probabilities honestly
          </p>
          <h2 class="mt-2 text-3xl font-semibold tracking-tight">
            A forecast is a range, not a promise
          </h2>
          <p class="mt-4 leading-7 text-muted-foreground">
            The model reflects the information available now. A single lineup
            change, injury, trade, or breakout week can change the strength
            assumptions and move the next forecast.
          </p>
          <div class="flex items-center gap-2 mt-6 text-sm font-medium">
            <ShieldCheck :size="17" class="text-primary" /> Re-run after league
            data and projections update
          </div>
        </div>
        <div class="p-6 border rounded-card bg-card">
          <div class="flex items-center justify-between gap-3 pb-4 border-b">
            <div>
              <p class="text-xs text-muted-foreground">Fourth &amp; Goal</p>
              <p class="mt-1 text-lg font-semibold">Projected finish</p>
            </div>
            <span class="text-3xl font-semibold text-primary">78%</span>
          </div>
          <dl class="grid grid-cols-2 mt-5 gap-x-6 gap-y-5">
            <div>
              <dt class="text-xs text-muted-foreground">Average wins</dt>
              <dd class="mt-1 font-semibold">8.2</dd>
            </div>
            <div>
              <dt class="text-xs text-muted-foreground">Likely range</dt>
              <dd class="mt-1 font-semibold">7–10 wins</dd>
            </div>
            <div>
              <dt class="text-xs text-muted-foreground">Average seed</dt>
              <dd class="mt-1 font-semibold">3.4</dd>
            </div>
            <div>
              <dt class="text-xs text-muted-foreground">Top-seed odds</dt>
              <dd class="mt-1 font-semibold">14%</dd>
            </div>
          </dl>
        </div>
      </div>
    ...
    <section class="max-w-4xl px-5 pb-16 mx-auto sm:pb-20">
      ...
      <h2 class="mt-2 text-3xl font-semibold tracking-tight">
        Fantasy football playoff odds questions
      </h2>
      ...
      >
        ...
        <div class="relative">
          <h2 class="text-2xl font-semibold tracking-tight">
            See every remaining path through your season
          </h2>
          <p class="max-w-2xl mt-2 leading-7 text-primary-foreground/80">
            Connect the league, open Season Forecast, and compare the playoff
            race after every completed week.
          </p>
        </div>
        <Button as-child size="lg" variant="secondary" class="relative shrink-0"
          ><RouterLink to="/">Run the forecast</RouterLink></Button
        >
      </div>
    </section>
  </PublicPageShell>
