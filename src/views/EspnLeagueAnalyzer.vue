...
const calculatedViews = [
  {
    ...
    description: "Compare record with scoring, expected wins, schedule strength, and roster projections.",
  },
  {
    icon: Trophy,
    title: "Playoff and season forecast",
    description: "Estimate playoff probability, seed range, and the remaining paths through the schedule.",
  },
  {
    icon: Newspaper,
    title: "Weekly reports",
    description: "Turn ESPN matchup results and lineup decisions into a recap built for the league chat.",
  },
  {
    icon: History,
...
const connectionPaths = [
  {
    ...
    description: "Select Public, paste the league ID from ESPN, choose the season, and load the league directly.",
  },
  {
    label: "Private ESPN league",
    requirement: "League ID + SWID + espn_s2",
    description: "Use the guided credential steps while signed in to ESPN. Credentials are stored locally in your browser and sent through ffwrapped only when loading that league from ESPN.",
  },
];
...
const faqs = [
  {
    ...
    answer: "Open the league in ESPN and look for the number following leagueId= in the page URL. Select the matching season in ffwrapped before importing.",
  },
  {
  ...
  },
  {
    ...
    answer: "ffwrapped stores them locally in the browser and only sends them through its service to ESPN when loading the private league. They should be treated like a password.",
  },
  {
...
<template>
  <PublicPageShell>
    <section class="relative overflow-hidden border-b">
      <div aria-hidden="true" class="absolute inset-x-0 top-0 h-64 bg-gradient-to-b from-primary/5 to-transparent"></div>
      <div class="relative grid gap-12 px-5 py-16 mx-auto max-w-6xl sm:py-24 lg:grid-cols-[minmax(0,1.04fr)_minmax(380px,0.96fr)] lg:items-center">
        <div>
          ...
          <div class="flex flex-wrap gap-x-5 gap-y-2 mt-6 text-sm text-muted-foreground">
            <span class="inline-flex items-center gap-1.5"><Check :size="14" /> Public and private leagues</span>
            <span class="inline-flex items-center gap-1.5"><Check :size="14" /> Season-aware import</span>
            <span class="inline-flex items-center gap-1.5"><Check :size="14" /> No spreadsheet setup</span>
          </div>
        </div>
        ...
        <div class="overflow-hidden border shadow-xl rounded-card bg-card shadow-black/10">
          <div class="flex items-center justify-between gap-4 px-5 py-4 border-b">
            <div class="flex items-center gap-3">
              <img src="/espnlogo.webp" width="28" height="28" alt="ESPN logo" />
              <div>
          ...
          <div class="p-5">
            <div class="inline-flex p-1 text-sm border rounded-control bg-muted/30">
              <span class="px-4 py-1.5 rounded-sm bg-background shadow-sm font-medium">Public</span>
              <span class="px-4 py-1.5 text-muted-foreground">Private</span>
            </div>
            <div class="flex gap-2 mt-4">
              <div class="flex-1 px-3 py-2 text-sm border rounded-control text-muted-foreground">League ID</div>
              <div class="px-3 py-2 text-sm border rounded-control text-muted-foreground">Season</div>
            </div>
            <div class="flex items-center justify-center h-10 mt-3 text-sm font-medium rounded-control bg-primary text-primary-foreground">Load ESPN league</div>
          </div>
          <div class="grid grid-cols-3 border-t divide-x bg-muted/30">
            <div class="p-4 text-center"><p class="text-lg font-semibold">12</p><p class="mt-1 text-xs text-muted-foreground">Teams</p></div>
            <div class="p-4 text-center"><p class="text-lg font-semibold">PPR</p><p class="mt-1 text-xs text-muted-foreground">Scoring</p></div>
            <div class="p-4 text-center"><p class="text-lg font-semibold">14</p><p class="mt-1 text-xs text-muted-foreground">Weeks</p></div>
          </div>
        </div>
      </div>
    </section>

    <section class="grid gap-12 px-5 py-16 mx-auto max-w-6xl sm:py-20 lg:grid-cols-2 lg:items-start">
      <div>
        <p class="text-sm font-medium text-primary">From ESPN</p>
        <h2 class="mt-2 text-3xl font-semibold tracking-tight">Bring the league structure with you</h2>
        <p class="mt-4 leading-7 text-muted-foreground">The import supplies the facts ffwrapped needs to understand the league on its own terms.</p>
        <ul class="mt-7 border-t">
          <li v-for="item in importedData" :key="item" class="flex items-center gap-3 py-4 text-sm border-b">
      ...
      <div>
        ...
        <h2 class="mt-2 text-3xl font-semibold tracking-tight">Add the context ESPN leaves between screens</h2>
        <div class="grid gap-px mt-7 overflow-hidden border bg-border rounded-card sm:grid-cols-2">
          <article v-for="view in calculatedViews" :key="view.title" class="p-5 bg-background">
    ...
    <section class="border-y bg-muted/30">
      <div class="px-5 py-16 mx-auto max-w-6xl sm:py-20">
        <div class="max-w-3xl">
          <p class="text-sm font-medium text-primary">Public and private leagues</p>
          <h2 class="mt-2 text-3xl font-semibold tracking-tight">A clear connection path for either ESPN setup</h2>
          <p class="mt-4 leading-7 text-muted-foreground">Private ESPN access needs more information than a public league. The page explains the difference before you enter anything.</p>
        </div>
        <div class="grid gap-4 mt-9 lg:grid-cols-2">
          <article v-for="path in connectionPaths" :key="path.label" class="p-6 border rounded-card bg-background sm:p-7">
            <div class="flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <LockKeyhole v-if="path.label.startsWith('Private')" :size="19" class="text-primary" />
                <ShieldCheck v-else :size="19" class="text-primary" />
                <h3 class="text-lg font-semibold">{{ path.label }}</h3>
              </div>
              <Badge variant="outline" class="font-normal">{{ path.requirement }}</Badge>
            </div>
            <p class="mt-5 text-sm leading-6 text-muted-foreground">{{ path.description }}</p>
          </article>
        </div>
      </div>
    </section>
    ...
    <section class="max-w-4xl px-5 py-16 mx-auto sm:py-20">
      ...
      <h2 class="mt-2 text-3xl font-semibold tracking-tight">ESPN league analyzer questions</h2>
